...
# http://127.0.0.1:5000//watch-information/or-method/Fossil?mobilenumber=88888&bandcolour=Black
@app.route('/watch-information/or-method/Fossil', methods=['GET'])
def home():
    mobileNumber = request.args.get('mobile')
    bandcolourNameFull = request.args.get('bandcolour')
    results = session.query(WatchInformation).filter(or_(WatchInformation.mobilenumber  == mobileNumber,WatchInformation.bandcolour == bandcolourNameFull)).all()
